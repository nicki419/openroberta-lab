# Unit-testing generated EdPy in Python

How to run the EdPy that the Lab generates for an Edison V2 program under CPython, and how to write unit tests
against it. It's the technical basis for the learner-facing unit-testing framework (see `CLAUDE.md`, "Project goal").

A working, tested reference implementation sits next to this document:

```
docs/ai/edpy-unit-testing/
  edtest/                       the harness (standard library only, CPython 3.8+)
    program.py                  EdProgram, run(), load() / Session, RunResult, arithmetic runtime
    transform.py                static EdPy guard, constant folding, source rewriting
  ...
  test_edtest.py                self-tests: the EdPy semantics the mock implements
  examples/
    clap_counter.xml            an example learner program (NEPO export)
    clap_counter.py             its EdPy, generated by the Lab, unmodified
    test_clap_counter.py        example tests at all levels
    test_golden_programs.py     the harness against the six Edison golden programs
```
...
**Status (2026-09-25):**
- All 46 tests pass with `unittest` (CPython 3.14.7) and with pytest 9.1.1. One of them is an intentional expected
  failure.
- The Level-0 test also ran against the real EdPy 1.2.11 compiler.
- Python 3.8 is the minimum by design (the harness uses `ast.Constant`), but only 3.14 was actually tested.
...
python -m pytest docs/ai/edpy-unit-testing          # also works, if pytest is installed
...
A complete test, against the example program in §4:

```python
from edtest import EdProgram, Robot

program = EdProgram.from_file('examples/clap_counter.py')
...
| Problem (all verified) | Consequence | How `edtest` handles it |
|---|---|---|
...
The harness works on the EdPy text. There are three ways to get it; all produce the same code.
...
2. **A throw-away Java test** (this is how `examples/clap_counter.py` was made). Put this class into
...
`examples/clap_counter.xml` is an expert-mode program. It was built from real block XML, and it passes the Lab's
...
`examples/test_clap_counter.py` tests it at every level (§5.1). The trace of a three-clap run, printed with
...
All levels use plain `unittest` (or pytest) assertions. The harness has no test DSL of its own; §10.3 proposes a data
format for learner-written and generated tests on top of it.
...
- **Names are NEPO names.** The generator emits `___x` for a variable or parameter `x`, and `____f` for a function
  `f`. The harness strips the prefixes everywhere. Helpers (`_diffDrive`, `sum`, …) and `obstacleDetectionOn` aren't
  exposed.
...
**Pitfalls every scenario author meets** (all shown by tests in `test_clap_counter.py`):
...
A property-testing library (e.g. Hypothesis) works as well, but the harness doesn't need one. Good generated inputs
are boundaries taken from the program: its comparison constants (100, 0), the 16-bit limits, 0 as a divisor, list
sizes as indices, and event times before, during and after the setup block (§10.4).
...
| Static guard rejections, literal range, constant folding | **verified with EdPy 1.2.11** (each case in `test_edtest.py`) |
...
## 8. What the harness found (verified runs)

These results come from `examples/test_clap_counter.py` and `examples/test_golden_programs.py`:
...
The transformed code keeps the EdPy file name and line numbers, so standard tracing works in EdPy lines (verified):

```python
import sys
hit = set()
def tracer(frame, event, arg):
    if frame.f_code.co_filename != program.filename:
        return None
    if event == 'line':
...
sys.settrace(tracer)
try:
    program.load().call('clampSpeed', 42)
finally:
    sys.settrace(None)
uncovered = [i for i, l in enumerate(program.lines, 1) if i not in hit and l.strip()]
```

- **Limitation:** the generator writes `if c: return v` on one line (`MethodIfReturn`). Line coverage therefore can't
  tell a taken return from a skipped one. Use the return value, or add branch coverage (e.g. `sys.monitoring` BRANCH
  events on CPython 3.12+).
- The same hook can count loop iterations, or record which comparisons were true, as input for test generation.

---

## 10. Towards the learner-facing framework

### 10.1 Where the tests run

The harness is pure standard-library Python, so there are two options:
- **Server:** the Java server calls a Python subprocess with the EdPy and a test description. The restricted builtins
  and the mock `Ed` are **not a sandbox**. Generated code comes from blocks and is constrained, but run it in a
  separate process with a wall-clock timeout and resource limits, and preferably in a container.
- **Browser:** the harness should run under Pyodide, because it only needs `ast`, `math`, `random` and `types`.
  **Unverified.** The browser sandbox isolates it, and no server load is added.

Either way, **the virtual clock keeps runs fast**. A 60-second robot scenario runs in milliseconds.

### 10.2 Reporting results to learners

- Errors and failed assertions point to EdPy lines. Learners see blocks, not code.
- **Mapping lines back to blocks needs a generator change.** For example, the code generator could record
  `line → block id` while emitting code. Block ids exist in the NEPO XML and in the simulator's stack-machine code, but
  not in the EdPy.
- EdPy accepts trailing comments, so `# id` comments are possible. But a comment-only block body is a syntax error
  (quirk #16), so ids must not be the only thing in a body.

### 10.3 A data format for tests (proposal, not implemented)

Learner-written tests (built with blocks or a form) and generated tests both need a neutral description that a small
interpreter turns into the calls above:

```json
{ "program": "clap_counter",
  "tests": [
    { "name": "speed is clamped", "kind": "function", "call": "clampSpeed", "args": [150], "expect": 100 },
    { "name": "three claps -> drive", "kind": "scenario",
      "world": [ {"clap": 1000}, {"clap": 2000}, {"clap": 3000} ], "max_time_ms": 10000,
      "expect": { "status": "finished", "vars": {"claps": 3},
                  "actions_include": [ {"Drive": [1, 10, 10]} ], "led_left_pattern": [true, false, true, false, true, false] } }
  ] }
```

Keep expectations to what §7 marks verified or derived, so that a passing test means the same thing on the robot.

### 10.4 Detecting what is worth testing (input for parts 2 and 3 of the goal)

Static signals come from the EdPy AST (`program` already parses it), or better, from the NEPO AST on the Java side:

| Signal | Suggests |
|---|---|
| `____` functions with parameters and a return value | Level-1 tests; parameter boundaries |
| comparison constants in conditions (`___speed > 100`) | boundary inputs: 99/100/101, 0/−1 |
| `/`, `%` with a variable divisor | a zero-divisor case |
| arithmetic on inputs, repeated `+=` in loops | overflow near ±32767 |
| `list[i]` with a variable index | index −1, `len`, `len`−1 |
| `while True:` + `Ed.Read…` (wait until) | scenario events: never, once, twice between reads, during the setup block |
| AND/OR with a latched sensor or a function call on the right | eager-evaluation cases (quirk #29) |
| actuator calls | what to observe (LED timeline, drive args, sounds) |
| a drive or motor block followed by `Ed.ReadClapSensor()` | claps during driving are discarded |

**Runtime signals** complement them: line coverage (§9), the trace (which sensors were polled and how often), the
final status (never finished → the scenario lacks an event), and runtime errors.

---

## 11. Extending and maintaining the harness

- **When the generator changes:**
  - Run `examples/test_golden_programs.py`. It fails if the mock stops understanding generated code.
...
- **Keep `test_edtest.py` green.** Each test there pins one semantic rule, and most rules were verified against the
  real compiler.
