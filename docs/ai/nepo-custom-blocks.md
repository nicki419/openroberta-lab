...
| 4 (optional) | Test the block's runtime behaviour: load the generated `.py` with the Python harness (`edpy-unit-testing.md`) and assert on `robot.trace`. If the block emits an `Ed` function the mock doesn't model yet, add `ed_<Name>` to `edtest/robot.py` (§11 there). | e.g. `EdProgram.from_file(p).run(Robot()).robot.calls('PlayMyBeep')` lists the beep with its argument |
