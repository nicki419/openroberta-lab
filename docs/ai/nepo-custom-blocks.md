...
| 4 (optional) | Test the block's runtime behaviour with the NEPO test framework (`nepo-unit-testing.md`): `TestSubject.from_lab(xml).run()` and look at `run.actions`. A new action block needs a decoder (`nepo-unit-testing.md` §8); an `Ed` function the engine doesn't model yet needs `ed_<Name>` in `NepoTest/nepotest/engine/robot.py` (`edpy-test-engine.md` §11). | the block shows up in `run.actions` with its NEPO values |
